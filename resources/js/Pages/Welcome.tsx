    ...
    try {
        ...
        const chime = (freq: number, startAt: number) => {
            ...
            gain.gain.linearRampToValueAtTime(
                0.06,
                ctx.currentTime + startAt + 0.14,
            );
            gain.gain.exponentialRampToValueAtTime(
                0.001,
                ctx.currentTime + startAt + 1.5,
            );
            osc.start(ctx.currentTime + startAt);
...
export default function Welcome({ auth }: PageProps) {
    ...
    useEffect(() => {
        goToBreakRef.current = goToBreak;
    }, [goToBreak]);
    useEffect(() => {
        goToFocusRef.current = goToFocus;
    }, [goToFocus]);
    ...
    return (
        <>
            <Head>
                ...
                <meta
                    name="description"
                    content="Free Pomodoro timer with gamification. Earn points, level up, and track your focus sessions. Build consistent productivity habits — no account required."
                />
                <meta
                    name="keywords"
                    content="pomodoro timer, productivity app, gamification, focus timer, time management, pomodoro technique, work sessions, level up, free timer"
                />
                <meta property="og:type" content="website" />
                <meta
                    property="og:title"
                    content="PomoBloom — Free Pomodoro Timer with Gamification"
                />
                <meta
                    property="og:description"
                    content="Free Pomodoro timer with gamification. Earn points, level up, and track your focus sessions. Build consistent productivity habits."
                />
                ...
                <meta
                    ...
                    content="Free Pomodoro timer with gamification. Earn points, level up, track your focus sessions."
                />
                ...
                <div className="relative">
                    <header className="flex items-center justify-between px-6 py-4">
                        ...
                        <nav className="hidden items-center gap-3 md:flex">
                            ...
                            <span className="select-none text-boundary/60">
                                |
                            </span>
                            <LocaleSwitcher />
                            <span className="select-none text-boundary/60">
                        ...
                        <div className="absolute left-0 right-0 top-full z-50 border-b border-boundary/40 bg-depth/95 px-6 py-5 shadow-xl shadow-black/40 backdrop-blur-sm md:hidden">
                            <div className="flex flex-col gap-4">
                                <div className="flex items-center gap-3">
                                    <LocaleSwitcher />
                                    <span className="select-none text-boundary/40">
                                        |
                                    </span>
                                    <ThemePicker />
                                </div>
                                ...
                                {user ? (
                                    <Link
                                        href={route("dashboard")}
                                        onClick={() => setMenuOpen(false)}
                                        className="rounded-full border border-ember/40 bg-ember/10 px-4 py-2 text-center text-sm font-semibold text-ember transition-all hover:bg-ember/20"
                                    >
                                        {t("nav.dashboard")} →
                                    </Link>
                                ) : (
                                    <div className="flex gap-2">
                                        <Link
                                            href={route("login")}
                                            onClick={() => setMenuOpen(false)}
                                            className="flex-1 rounded-full border border-boundary/60 px-3 py-2 text-center text-sm font-medium text-whisper transition-all hover:text-moonbeam"
                                        >
                                            {t("nav.login")}
                                        </Link>
                                        <Link
                                            href={route("register")}
                                            onClick={() => setMenuOpen(false)}
                                            className="flex-1 rounded-full bg-ember px-3 py-2 text-center text-sm font-semibold text-white transition-all hover:brightness-110"
                                        >
                                            {t("nav.register")}
                                        </Link>
                                    </div>
                ...
                <main className="flex flex-1 flex-col items-center px-4 pb-16 pt-4">
                    <div className="w-full max-w-xs rounded-3xl border border-whisper/10 bg-depth shadow-2xl shadow-black/70">
                        ...
                        <div className="relative h-0.5 overflow-hidden rounded-t-3xl">
                            <div
                                className={`absolute inset-0 bg-gradient-to-r from-ember via-bloom/50 to-ember transition-opacity duration-700 ${isFocus ? "opacity-100" : "opacity-0"}`}
                            />
                            <div
                                className={`absolute inset-0 bg-gradient-to-r from-bloom/80 via-bloom/30 to-bloom/80 transition-opacity duration-700 ${isFocus ? "opacity-0" : "opacity-100"}`}
                            />
                        </div>
                        ...
                        <div className="px-8 pb-8 pt-6">
                            ...
                            <div className="mb-5 flex items-center justify-between">
                                <p
                                    className={`text-xs font-semibold uppercase tracking-[0.25em] transition-colors duration-700 ${isFocus ? "text-ember" : "text-bloom"}`}
                                >
                                    {isFocus
                                        ? t("timer.focus")
                                        : t("timer.short_break")}
                                ...
                                >
                                    {notifPermission === "denied" ? (
                                        ...
                                        <div className="flex items-start gap-2">
                                            <svg
                                                className="mt-0.5 shrink-0 text-ember/70"
                                                width="12"
                                                height="12"
                                                viewBox="0 0 24 24"
                                                fill="none"
                                    ...
                                    <>
                                        <button
                                            ...
                                            className={`rounded-full px-12 py-3 text-sm font-semibold uppercase tracking-widest text-white shadow-lg transition-all hover:brightness-110 active:scale-95 ${
                                                isFocus
                                                    ? "bg-ember shadow-ember/30"
                                                    : "bg-bloom shadow-bloom/30"
                                            }`}
                                        >
                                            {t("timer.start")}
                                        </button>
